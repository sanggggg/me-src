...
> z_id column 이 Z 테이블에 대한 제약을 거는 중에 Z테이블은 가 레코드가 없으니 제약을 확인하는데 큰 시간이 들지 않을거라 큰 문제 없지 않을까

Z 테이블은 방금 막 추가된 테이블이며, 따라서 이에 대해서 제약을 확인하는 절차를 거칠 필요가 없다. MySQL 이 COPY 알고리즘을 택한 이유는 정합성 검사가 필요하기 때문이었고, 그렇다면 정합성 검사를 skip 한다면 COPY 가 아닌 INPLACE 알고리즘을 쓸 수 있지 않을까? 공식 문서에서 아래 문장을 확인할 수 있었다.
...
alter table Y add constraint FK_Z foreign key (z_id) references Z(id) algorithm=INPLACE;
