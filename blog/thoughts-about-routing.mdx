...
date: 2022-09-04
tag: ribs, android, iOS
author: 상민
---

최근 [RIBs](https://github.com/uber/RIBs), [Jetpack Navigation](https://developer.android.com/guide/navigation?gclid=Cj0KCQjwmdGYBhDRARIsABmSEePbIHuVd2hgF1c6RZVZslJE2AaofaPO7blK4Fegspw-Qi5YAbw43aYaAmrmEALw_wcB&gclsrc=aw.ds) 등의 Routing Library 를 쓰다가 든 생각을 휘날려 정리해봅니다.
...
이 글에서는 RIBs, Jetpack Navigation (with MVVM Fragment) 을 둘 다 써 보면서 느낀 Logical 한 노드를 통한 Routing 이 좋은 점을 적으려고 한다.
...
class HomeUI(
    ...
    fun onClickDetail(detailItem: DetailItem) {
        if (homeLogicProvider.isRouteToDetailAllowed(detailItem)) { // 실제 그 결과를 통해서 라우팅을 할 지 말지 등을 결정할 수도 있을테고...
            navigator.routeTo(DetailUI.Identifier)
        }
    }
}
