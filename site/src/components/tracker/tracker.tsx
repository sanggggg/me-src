...
export default function TrackerContent({
  ...
  rawData: Array<{ day: Date; tags: Array<Tag> }>;
  start: Date;
  end: Date;
}) {
  ...
  const monthIndicator = modifiedData[modifiedData.length - 1]
    ...
    .reduce<Array<{ month: number; count: number }>>((acc, cur) => {
...
function transposeArray<T>(array: Array<Array<T>>): Array<Array<T>> {
  // Calculate the number of rows and columns.
  const numRows = array.length;
  const numCols = array[0].length;

  // Create a new array with the transposed dimensions.
  const transposedArray: Array<Array<T>> = new Array(numCols);
